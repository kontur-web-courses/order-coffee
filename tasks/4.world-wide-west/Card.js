const Card = function () {
    function Card(name, maxPower, image) {
        ...
        definePower(this, maxPower);

        this.view = new CardView();
        this.updateView();
    }

    // Определяет свойства для силы.
    function definePower(card, maxPower) {
        card.currentPower = maxPower;
        card.maxPower = maxPower;
    }
    ...
    // Строит описания особенностей карты.
    // Можно переопределить в наследниках.
    // Позволяет задавать описания способностей различных карт.
