...
document.querySelector('.submit-button').addEventListener('click', e => {
    ...
    let forms = document.querySelectorAll('.beverage');
    content.innerHTML = `Вы заказали ${forms.length} ${declOfNum(forms.length, ['напиток', 'напитка', 'напитков'])}`;
...
function constructTable() {
    ...
    for(form of document.querySelectorAll('.beverage')){
        ...
        for(additionalObj of form.querySelectorAll('input[type=checkbox]:checked + span'))
